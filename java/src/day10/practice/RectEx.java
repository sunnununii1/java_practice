...
class Rect1{
	...
	public void move(int x, int y) {
		int dx = leftUpX - x;	// 이동한 x좌표 거리 계산
		int dy = leftUpY - y;	// 이동한 y좌표 거리 계산
...
class Rect2{
	...
	//메서드
		 ...
		 */
		public void move(int x, int y) {
			int dx = leftUp.getX() - x;	// 이동한 x좌표 거리 계산
			int dy = leftUp.getY() - y;	// 이동한 y좌표 거리 계산
			
			leftUp.move(x, y);
			leftUp.move(rightDown.getX() - dx ,rightDown.getY() - dy);
		}
